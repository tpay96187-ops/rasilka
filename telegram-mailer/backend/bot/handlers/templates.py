...
async def list_templates(callback: CallbackQuery):
    ...
    if not templates:
        await callback.message.edit_text("📝 Шаблоны отсутствуют.", reply_markup=templates_list_kb(templates))
    else:
        await callback.message.edit_text("📝 Ваши шаблоны:", reply_markup=templates_list_kb(templates))
    await callback.answer()

# === Обработчик инлайн-кнопки "Создать шаблон" ===
@router.callback_query(F.data == "new_template")
async def new_template_callback(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer("Введите название шаблона:")
    await state.set_state(TemplateStates.waiting_name)
    await callback.answer()

# === Команда /new_template (альтернативный способ) ===
...
async def template_content(message: Message, state: FSMContext):
    ...
    templates = await get_templates()
    await message.answer("📝 Ваши шаблоны:", reply_markup=templates_list_kb(templates))
...
async def show_template(callback: CallbackQuery):
    ...
    text = f"📝 *{tpl.name}*\n{status}\n\n📄 Содержание:\n{tpl.content[:500]}"
    await callback.message.edit_text(text, reply_markup=template_actions_kb(template_id, tpl.is_active))
    await callback.answer()
...
async def delete_template_final(callback: CallbackQuery):
    ...
    templates = await get_templates()
    if not templates:
        await callback.message.answer("📝 Шаблоны отсутствуют.", reply_markup=templates_list_kb(templates))
    else:
        await callback.message.answer("📝 Ваши шаблоны:", reply_markup=templates_list_kb(templates))
