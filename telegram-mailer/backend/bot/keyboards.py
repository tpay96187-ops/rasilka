...
def groups_selection_kb() -> InlineKeyboardMarkup:
    """Клавиатура для выбора режима выбора групп"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Выбрать ВСЕ группы", callback_data="groups_select_all")],
        [InlineKeyboardButton(text="📝 Выбрать отдельно (по ID)", callback_data="groups_select_manual")],
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="cancel_campaign_creation")]
    ])
